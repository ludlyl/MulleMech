...
void Miner::OnStep(Builder* builder_) {
    ...
    Units workers = gAPI->observer().GetUnits(sc2::Unit::Self);
    auto itr2 = std::remove_if(workers.begin(), workers.end(), [commandCenters](const Unit* u) {
        if(!IsCombatUnit()(*u)){
            if((u->unit_type.ToType() == sc2::UNIT_TYPEID::TERRAN_SCV && u->mineral_contents == 5) || (u->unit_type.ToType() == sc2::UNIT_TYPEID::TERRAN_MULE && u->mineral_contents == 25)){
                gAPI->action().Cast(u, sc2::ABILITY_ID::SMART, commandCenters.GetClosestUnit(u->pos));
            }
        }
        return IsCombatUnit()(*u);
    });
