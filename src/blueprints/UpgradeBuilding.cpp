...
bool bp::UpgradeBuilding::CanBeBuilt(const Order* order_) {
...
bool bp::UpgradeBuilding::Build(Order* order_) {
    auto pos = FindBuildLocation(order_);
    if (pos.has_value())
...
std::optional<sc2::Point3D> bp::UpgradeBuilding::FindBuildLocation(const Order* order_) const {
    Timer timer;
    timer.Start();
    ...
    gHistory.debug() << "Trying to calculate placement for upgrade building took " << ms << " ms" << std::endl;
